...
/* Verifi current password */
router.post('/verifyCurrentPassword', function (req, res, next) {
    if (verifySession(req)) {
        User.findById(req.session.userId).then(function (user) {
            if (user) {
                if (bcrypt.compareSync(req.body.pwd, user.password)) {
                    res.send({etat: true, message: 'ok'})
                } else {
                    res.send({etat: false, message: allConfig.get('login_message:bad_password')})
                }
            } else {
                res.send({etat: false, message: allConfig.get('session_message:reconnect')})
            }
